...
function main() {
  ...
  for (const f of resourceFiles) {
    ...
    if (data?.workflows) {
      for (const wf of Object.values(data.workflows)) {
        ...
        if (!existsSync(fullPath)) {
          warnings.push(`${relative(ROOT, f)}: workflow ref "${wf}" does not exist`);
        }
      }
    }
    if (data?.console?.list_route) {
      ...
      if (!existsSync(fullPath)) {
        warnings.push(`${relative(ROOT, f)}: list_route ref "${data.console.list_route}" does not exist`);
      }
    }
  }
  ...
  for (const f of routeFiles) {
    ...
    if (data?.screen?.primary_resource && !resourceIds.has(data.screen.primary_resource)) {
      warnings.push(`${rel}: primary_resource "${data.screen.primary_resource}" has no resource file`);
    }
    if (data?.operations) {
      for (const op of data.operations) {
        if (op.workflow) {
          ...
          if (!existsSync(fullPath)) {
            warnings.push(`${rel}: workflow ref "${op.workflow}" does not exist`);
          }
        }
      }
    }
    ...
    function checkNavRefs(nodes) {
      for (const node of nodes || []) {
        if (node.route_file) {
          ...
          if (!existsSync(fullPath)) {
            warnings.push(`console-tree.yaml: route_file "${node.route_file}" does not exist`);
          }
        }
